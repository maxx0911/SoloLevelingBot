...
class HelpPageSource(menus.ListPageSource):
    ...
    def __init__(self, help_command, cmds):
        # entries = [(cog, len(sub)) for cog, sub in commands.items()]
        # entries.sort(key=lambda t: (t[0].qualified_name, t[1]), reverse=True)
        super().__init__(
            entries=sorted(cmds.keys(), key=lambda c: c.qualified_name), per_page=6
        )
        self.commands = cmds
        self.help_command = help_command
        self.prefix = help_command.clean_prefix

    def format_commands(self, cog, cmds):
    ...
    async def format_page(self, menu, cogs):
        ...
        for cog in cogs:
            cmds = self.commands.get(cog)
            if cmds:
                value = self.format_commands(cog, cmds)
...
class GroupHelpPageSource(menus.ListPageSource):
    def __init__(self, group, cmds, *, prefix):
        super().__init__(entries=cmds, per_page=6)
        self.group = group
        self.prefix = prefix
...
class PaginatedHelpCommand(commands.HelpCommand):
    ...
    async def send_bot_help(self, mapping):
        ...
        menu = HelpMenu(HelpPageSource(self, all_commands))
...
class Help(commands.Cog):
    """overwrite of the default help command"""
